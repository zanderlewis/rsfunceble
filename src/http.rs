...
pub async fn check_http(url: &str, verbose: bool) -> Result<(bool, bool), String> {
    ...
    let response = client.get(url).send().await.map_err(|e| format!("HTTP Status Failed: {}", e))?;
    ...
    if verbose {
        if is_active {
            println!("HTTP check for {} succeeded with status code {}", url, status_code);
        } else if is_inactive {
            println!("HTTP check for {} failed with status code {}", url, status_code);
        } else {
            println!("HTTP check for {} returned status code {}", url, status_code);
        }
